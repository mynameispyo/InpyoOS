...
.main_loop:
	...
	mov cx, 0						; Reset the time counter
...
.clear_loop:
	mov dword [edi + ebx], 0		; Use dwords to make it faster
	add ebx, 4
	cmp ebx, 8000
	jne .clear_loop
...
clear_counter:
	...
	mov edi, 0A0000h + 7600 - 65536	; 7600: X=0, Y=190
...
draw_sprite:
	...
	or edi, 0A0000h - 65536			; Point to the video RAM
	mov cl, 0
		
.loop:
	mov al, [si]					; Get a byte from the sprite data
	
	call extend_byte				; Expand it
	
	push cx
	mov cl, 0						; Line draw counter
	
.draw_loop:
	push ebx
	ror ebx, 16
	mov [edi], bl					; Draw the 1st byte
	pop ebx
	
	push ebx
	ror ebx, 8
	mov [edi + 1], bl				; Draw the 2nd byte
	pop ebx
	
	mov [edi + 2], bl				; Draw the 3rd byte
	
	add edi, 40						; Point to the next line
	inc cl
	cmp cl, 3						; Have we drawn the line?
	jne .draw_loop
	...
	inc cl
	cmp cl, 8						; Are we done with the sprite?
	jne .loop
...
extend_byte:
	pushad
	and eax, 000000FFh				; We only need the low 8 bits
	mov cl, 0						; Counter
	mov ebx, 0						; Temporary integer
	
.decode_loop:
	push bx
	mov dx, 0						; Not to interfere with DIV
